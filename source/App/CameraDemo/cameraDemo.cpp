...
int matchID(FACE3D_Type* gf, float* features, int numSavedFaces);
	...
	while(1)
      {
		...
		{	
			...
			if ( numSavedFaces > 0)
			{
				matchedFaceID = matchID(&gf, gf.faceFeatures, numSavedFaces);
			}
			...
			else
			{
				...
				for ( len = 0; len < 260; len++)
				{
					...
					tmpName[len] = names[matchedFaceID-1][len];
					if (names[matchedFaceID-1][len] == 0)
						break;
				}
		...
		if ( key == 'c')
		{
			printf("Input name!\n");
			gets(tmpGets);
			strcpy(names[numSavedFaces], tmpGets);
			printf("Please input ID\n");
			cin >> matchedFaceID;
			memcpy(gf.bufferFaceFeatures[numSavedFaces].feature, gf.faceFeatures, sizeof(float)*TOTAL_FEATURE_LEN);
			gf.bufferFaceFeatures[numSavedFaces].id = matchedFaceID;
			numSavedFaces++;
		}
...
int matchID(FACE3D_Type* gf, float* features, int numSavedFaces)
{
	...
	float dist, minDist, tmpDist;
	int i, j, k, matchID;

	minDist = 9999999999999999999999.0;
	matchID = 0;
	for ( i = 0; i < numSavedFaces; i++)
	{
		dist = 0;
		for ( j = 0; j < TOTAL_FEATURE_LEN; j++)
		{
			tmpDist			= (features[j])-(bufferFaceFeatures[i].feature[j]);
			if( !(features[j] == 0 && bufferFaceFeatures[i].feature[j] ==0))
			{
				dist += (tmpDist * tmpDist)/(features[j] + bufferFaceFeatures[i].feature[j]);
			}
		}
		if ( dist < minDist)
		{
			matchID = bufferFaceFeatures[i].id;
			minDist = dist;
		}
	}
