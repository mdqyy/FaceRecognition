...
*/
#ifndef BINARY_FORMAT_C_INCLUDED 
#define BINARY_FORMAT_C_INCLUDED 
...
{// {{{
	...
	*comment = 0;
	...
	words = (SVM_WORD *)my_malloc(sizeof(SVM_WORD)*(max_words_doc+1));
	dnum=0;
...
int read_feature(FILE *docfl, SVM_WORD *words, double *label,
...
{// {{{
...
#else
		SVM_WORD *words = (SVM_WORD *)my_malloc(sizeof(SVM_WORD)*(model->totwords+1));
#endif
	...
	fclose(modelfl);
	if(verbosity>=2) {
		//fprintf(stdout, "SVM: OK. (%d support vectors read)\n",(int)(model->sv_num-1));
	}
	return(model);
}// }}}
