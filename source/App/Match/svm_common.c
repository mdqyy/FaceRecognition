...
/*long   format; changed by Tony, only binary is format is supported */   /* binary format (0-1) */
long   kernel_cache_statistic;
double X2_scale_param = 0.1;
     ...
     /*            not be larger than the weight vector!               */
{
  ...
  //Te be revised abouv: a*(b1+b2+b3..)=a*b1+a*b2+a*b3+.. but LHS is much faster.....
  return(sum-model->b);
}
...
SVECTOR *create_svector(SVM_WORD *words,FNUM n_words,char *userdefined,double factor)
{
  SVECTOR *vec;
  long    fnum,i;
...
double GetScaleParam(DOC **docs,int totwords, int totdoc)
{
	DOC *pDoc = NULL;
	double temp_sum = 0.0;
	...
	for(i=0; i<totdoc - 1; i++){
		...
		for(j=i+1;j<totdoc; j++){
			temp_sum += X2_Dist(docs[i]->fvec, docs[j]->fvec);
		}
	}
	return (temp_sum*2/(totdoc*(totdoc - 1)));
}
//Important: we omit the coefficient 1/2 for param A and distance representation simutaliously
__inline double X2_Dist(SVECTOR *a, SVECTOR *b)
{
    register CFLOAT sum=0;
    register float fEpsilion = 0.00001;
#ifdef DENSE
    FVAL *fa=a->words, *fb=b->words;
    FNUM n=a->n_words, i;
    for (i=0; i<n; i++) {
       sum += (fa[i] - fb[i])*(fa[i] - fb[i])/(fa[i] + fb[i] + fEpsilion);
    }
 //   printf("Refactor\n");
#else
    register WORD *ai,*bj;
	...
	sum+=((CFLOAT)(ai->weight) - (CFLOAT)(bj->weight))*((CFLOAT)(ai->weight) - (CFLOAT)(bj->weight))/((CFLOAT)(ai->weight) + (CFLOAT)(bj->weight) + fEpsilion);
	ai++;
	bj++;
      }
    }
