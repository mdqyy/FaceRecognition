...
void CNNIO::init(int outNode, int width, int height, ConvNN *CNN)
{
	...
	for( k = 0, layer = CNN->m_cnn->network->layers; k < 4/*n_layers*/; k++, layer = layer->next_layer )
	{
						...
						int  P_K, int P_a, int P_s*/
{
	...
	n_input_planes  = 1;
	input_height    = FACECLIPHEIGHT;
	input_width     = FACECLIPWIDTH;
	n_output_planes = 6;
	//n_output_planes = 7;
	output_height   = (FACECLIPHEIGHT-3)/2;
	output_width    = (FACECLIPWIDTH-3)/2;
	init_learn_rate = LEARNINGRATE;
	...
	//n_input_planes  = 7;
	n_input_planes  = 6;
	input_height    = (FACECLIPHEIGHT-3)/2;
	input_width     = (FACECLIPWIDTH-3)/2;
	n_output_planes = 50;
	//n_output_planes = 60;
	...
	n_output_planes = CONNECTIONNODE;
	init_learn_rate = LEARNINGRATE;
	a = 1;
	s = 1;
...
void ConvNN::writeCNNPara(string outFile)
{
	FILE *fpweights;
...
void ConvNN::LoadCNNPara(string inFile/*string inFile, int nSample, float maxIter, 
									  int input_planes, int input_height, int input_width, int output_planes, int output_height,
									  int output_width, float learn_rate, int learn_type, int delat_w_increase_type, 
									  int  P_K, int P_a, int P_s*/)
{
