...
void  featureLBPHist(IplImage * img,CvMat *features_final)
{
    ...
    int Nx = (int)floor((double)(img->width )/35);
    int Ny= (int)floor((double)(img->height)/30);
...
IplImage * SQI(CvMat * filter,IplImage *  image,int size)
{
    ...
    for ( i = 0; i < image->height; i++)
    {
        for ( j = 0; j <  image->width; j++)
        {
            ...
            indexl=i- int(floor((double)(size)/2));
            if (indexl < 0)
                indexl =0;
            indexl1=i+ int(floor((double)(size)/2));
            if (indexl1 >= height)
                indexl1 = height- 1;

            indexm=j- int(floor((double)(size)/2));
            if (indexm < 0)
                indexm = 0;

            indexm1 =j+ int(floor((double)(size)/2));
            if (indexm1 >= width)
                indexm1= (width)  - 1;
            ...
            for ( l = 0; l < size; l++)
            {
                indexl=i+ l - int(floor((double)(size)/2));
                if (indexl < 0)
                    indexl = abs(indexl) - 1;
                if (indexl >= height)
                ...
                for ( m = 0; m < size; m++)
                {
                    indexm=j+ m - int(floor((double)(size)/2));
                    if (indexm < 0)
                        indexm = abs(indexm) - 1;
                    if (indexm >= width)
...
int peakToSideLobeRatio(CvMat*maceFilterVisualize,IplImage *img,int  SIZE_OF_IMAGE)
{
	...
	double temp1 = floor((45.0/64.0)*(double)SIZE_OF_IMAGE);
    int radius1=(int)(temp1);
	temp1 = floor((27.0/64.0)*(double)SIZE_OF_IMAGE);
    int radius2=(int)(temp1);
...
//    cvPow( image_Im, image_Im, 2.0);
    ...
    for (l=0;l<SIZE_OF_IMAGE_2X;l++)
    {
        for (m=0;m<SIZE_OF_IMAGE_2X;m++)
        {
            ...
            double rad=sqrt((pow((double)(m-SIZE_OF_IMAGE),2)+pow((double)(l-SIZE_OF_IMAGE),2)));
            if (rad<rd1)
            {
